...
// NOTE: テストファイルがTestMainを実装している場合、GoはテストをTestMain(m)を呼び出す。
// パッケージ内全てのテストに適用される設定を行う
...
func TestServer(t *testing.T) {
	scenarios := map[string]func(t *testing.T, rootClient api.LogClient, nobodyClient api.LogClient, config *Config){
		"produce/consume a mesdsage to/from the log succeeds": testProduceConsume,
		"produce/consume stream succeeds":                     testProduceConsumeStream,
		"consume past log voundary fails":                     testConsumePastBoundary,
		"unauthorized fails":                                  testUnauhtorized,
	}
...
func testProduceConsume(t *testing.T, client, _ api.LogClient, config *Config) {
	...
	consume, err := client.Consume(ctx, &api.ConsumeRequest{Offset: want.Offset})
	require.NoError(t, err)
...
func testUnauhtorized(t *testing.T, _, client api.LogClient, config *Config) {
	ctx := context.Background()
